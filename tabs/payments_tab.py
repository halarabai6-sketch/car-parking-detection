...
class PaymentsTab:
    ...
    def _build(self):
        ...
        ctk.CTkButton(top, text="+ Enregistrer Paiement", width=180,
                      fg_color="#00d4aa", hover_color="#00b894",
                      text_color="#000", command=self._add_dialog).pack(side="left", padx=5)
        ctk.CTkButton(top, text="\U0001f504 Actualiser", width=120,
        ...
        for i, (text, w) in enumerate([("Client", 3), ("Matricule", 2), ("Payé", 1), ("Reste", 1),
                                         ("Mois", 2), ("Date", 2), ("", 1)]):
    ...
    def refresh(self):
        ...
        if not payments:
            ctk.CTkLabel(self.list_frame, text="Aucun paiement enregistré.",
    ...
    def _render_row(self, payment):
        ...
        for i, w in enumerate([3, 2, 1, 1, 2, 2, 1]):
            row.columnconfigure(i, weight=w)

        name = f"{payment.get('first_name', '')} {payment.get('last_name', '')}"
        amount_paid = payment['amount']
        amount_owed = max(0, 500.0 - amount_paid)
        
        values = [name, payment.get("plate_number", "—"),
                  f"{amount_paid:.2f} MAD", f"{amount_owed:.2f} MAD", payment["month_covered"],
                  payment["payment_date"]]
        for i, val in enumerate(values):
            color = "#fdcb6e" if i == 3 and amount_owed > 0 else "#fff"
            ctk.CTkLabel(row, text=val, font=ctk.CTkFont(size=12), text_color=color,
    ...
    def _add_dialog(self):
        ...
        if not clients:
            ...
            dialog.title("Aucun Client")
            dialog.geometry("300x130")
            dialog.grab_set()
            dialog.attributes("-topmost", True)
            ctk.CTkLabel(dialog, text="Ajoutez des clients d'abord !",
                         font=ctk.CTkFont(size=14)).pack(pady=30)
            ctk.CTkButton(dialog, text="OK", command=dialog.destroy).pack()
            return

        dialog = ctk.CTkToplevel(self.parent)
        dialog.title("Enregistrer Paiement")
        dialog.geometry("420x400")
        dialog.resizable(False, False)
        dialog.grab_set()
        dialog.attributes("-topmost", True)

        ctk.CTkLabel(dialog, text="Enregistrer Paiement",
        ...
        # Amount
        ctk.CTkLabel(dialog, text="Montant Payé (Frais mensuel = 500 MAD):", anchor="w",
                     font=ctk.CTkFont(size=12)).pack(fill="x", padx=30, pady=(5, 0))
        amount_entry = ctk.CTkEntry(dialog, width=350, placeholder_text="ex: 500.00")
        amount_entry.pack(padx=30, pady=(0, 8))

        # Month covered — Year + Month dropdowns
        ctk.CTkLabel(dialog, text="Mois Couvert:", anchor="w",
                     font=ctk.CTkFont(size=12)).pack(fill="x", padx=30, pady=(5, 0))
        ...
        def _save():
            ...
            if idx < 0:
                msg_lbl.configure(text="Veuillez sélectionner un client !")
                return
            try:
                amount = float(amount_entry.get())
                if amount < 0 or amount > 500:
                    msg_lbl.configure(text="Le montant doit être entre 0 et 500 !")
                    return
            except ValueError:
                msg_lbl.configure(text="Veuillez entrer un montant valide !")
                return

            # Build YYYY-MM from dropdowns
            year = year_menu.get().strip()
            month_str = month_menu.get().strip()[:2]  # "01 - Janvier" → "01"
        ...
        ctk.CTkButton(dialog, text="\u2714  Sauvegarder Paiement", height=40, width=250,
                      fg_color="#00d4aa", hover_color="#00b894",
